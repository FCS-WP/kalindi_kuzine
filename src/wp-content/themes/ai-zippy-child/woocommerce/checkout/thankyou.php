...
/**
 * Thank You Page — AI Zippy Child Override (Split Order Support)
 *
 * Displays all related orders (parent and children) on the thank you page.
 *
 * @var WC_Order $order
 */
...
$related_ids = [];
if (class_exists('\AiZippyChild\Inc\SplitOrder')) {
	$related_ids = \AiZippyChild\Inc\SplitOrder::get_related_orders($order->get_id());
} else {
	$related_ids = [$order->get_id()];
}

// Sort related orders: Parent first, then sub-orders
sort($related_ids);
		...
		<?php else : ?>
			...
			<div class="az-thankyou__header">
				...
				<?php if ($order->get_billing_email()) : ?>
					<p class="az-thankyou__email-note"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
							<rect x="2" y="4" width="20" height="16" rx="2" />
							<polyline points="22,4 12,13 2,4" />
						</svg> <?php printf(esc_html__('Order confirmation sent to %s', 'ai-zippy'), '<strong>' . esc_html($order->get_billing_email()) . '</strong>'); ?></p>
				<?php endif; ?>
			</div>

			<!-- Loop Through All Related Orders -->
			<?php foreach ($related_ids as $rel_id) :
				$rel_order = wc_get_order($rel_id);
				if (!$rel_order || $rel_order->get_item_count() === 0) continue;

				$date = $rel_order->get_meta('_billing_date');
				$time = $rel_order->get_meta('_billing_time');
				$mode = $rel_order->get_meta('_billing_order_mode');
				...
				$time_display = '';
				if ($time) {
					$time_data = maybe_unserialize($time);
					if (is_array($time_data)) {
						$time_display = ($time_data['from'] ?? '') . ' - ' . ($time_data['to'] ?? '');
					} else {
						$time_display = $time;
					}
				}
			?>
				<div class="az-thankyou__card az-thankyou__card--split">
					<div class="az-thankyou__card-header">
						<h3 class="az-thankyou__card-title">
							<?php if (count($related_ids) > 1) : ?>
								<span class="az-thankyou__badge"><?php echo ucfirst($mode); ?></span>
							<?php endif; ?>
							<?php echo $formatted_date ? esc_html($formatted_date) : esc_html__('Order Summary', 'ai-zippy'); ?>
							<?php if ($time_display) echo ' <small>(' . esc_html($time_display) . ')</small>'; ?>
						</h3>
						<?php if (count($related_ids) > 1) : ?>
							<span class="az-thankyou__sub-order-num">#<?php echo esc_html($rel_order->get_order_number()); ?></span>
						<?php endif; ?>
					</div>

					<div class="az-thankyou__items">
						<?php foreach ($rel_order->get_items() as $item_id => $item) :
							$product = $item->get_product();
							$qty     = $item->get_quantity();
							$total   = $rel_order->get_formatted_line_subtotal($item);
						?>
					...
					<div class="az-thankyou__totals">
						<div class="az-thankyou__totals-row">
							...
							<span><?php echo wp_kses_post($rel_order->get_subtotal_to_display()); ?></span>
						</div>

						<?php if ($rel_order->get_shipping_method()) : ?>
							<div class="az-thankyou__totals-row">
								<span><?php esc_html_e('Shipping', 'ai-zippy'); ?></span>
								<span>
									<?php
									$shipping_total = (float) $rel_order->get_shipping_total();
						...
						<div class="az-thankyou__totals-row az-thankyou__totals-row--total">
							...
							<span><?php echo wp_kses_post($rel_order->get_formatted_order_total()); ?></span>
						</div>
					</div>
				</div>
			<?php endforeach; ?>

			<!-- Delivery Details (Combined) -->
			<?php
...
<style>
	...
	.az-thankyou__sub-order-num {
		font-size: 13px;
		color: #888;
	}

	.az-thankyou__card--split {
		margin-bottom: 24px;
	}
</style>
